...
//        mRtcEngine = null;
    ...
    private void setUpEngine(Intent intent) {
        ...
        final String userFullName = intent.getStringExtra(Constant.USER_FULL_NAME);
        try {
            mRtcEngine = RtcEngine.create(getApplicationContext(), appId, new IRtcEngineEventHandler() {
                ...
                public void onJoinChannelSuccess(String channel, int uid, int elapsed) {
                    ...
                    try {
                        msgObj.put("event","screen_share");

                        JSONObject dataObj = new JSONObject();
                        dataObj.put("uid", uid);
                        dataObj.put("userFullName", userFullName);
                        dataObj.put("status", "connected");
                    ...
                    int streamId = mRtcEngine.createDataStream(dataStreamConfig);

                    mRtcEngine.sendStreamMessage(streamId, msgObj.toString().getBytes(Charset.forName("UTF-8")));
                }
