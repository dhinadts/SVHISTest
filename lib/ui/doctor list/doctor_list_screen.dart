...
import '../doctor_schdule_screen.dart';

import '../booking_appointment/models/doctor.dart';

class DoctorListScreen extends StatefulWidget {
  final String title;
  final Map patientDetails;

  const DoctorListScreen({Key key, this.title, this.patientDetails})
      : super(key: key);
...
class _DoctorListScreenState extends State<DoctorListScreen> {
  ...
  Future getDoctorsList() async {
    ...
    if (ValidationUtils.isSuccessResponse(response.statusCode)) {
      ...
      if (jsonData != null) {
        setState(() {
          doctorList = jsonMapData["doctorList"].map<Doctor>((x) {
            return Doctor.fromJson(x);
          }).toList();
        });
      }
    }
        ...
        child: Row(
          ...
          children: [
            Container(
              ...
              padding:
                  EdgeInsets.symmetric(horizontal: AppUIDimens.paddingXSmall),
              child: TextFormField(
                focusNode: focusNode,
    ...
    if (response.statusCode == WebserviceHelper.WEB_SUCCESS_STATUS_CODE) {
      try {
        ...
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (BuildContext context) => DoctorSchedulerScreen(
                title: "Schedule", userInfo: userDoctorInfo),
          ),
        );
